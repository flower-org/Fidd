...
На базовом уровне, сами публичные ключи подписчиков нигде не должны фигурировать в открытом виде, их должно быть невозможно выяснить
...
Для того, чтобы узнать наименование своего Fidd Key файла, подписчику необходимо вычислить т.н. "футпринт", используя
свой публичный ключ. Для этого следует закриптовать публичным ключом конкатенацию (str(Message_Number) + str(Message_File_Size)).
Далее, полученный "футпринт" следует преобразовать в строку при помощи алгоритма Base36, что ограничит символы строки
набором, пригодным для формирования имени файла.  
Идеально было бы здесь задействовать еще и публичный ключ паблишера, но на данном этапе не до конца понятно, будет ли 
этот ключ обязательным, или уникальным, или же будет ли он константным; к тому же теоретически мы не исключаем наличия 
более чем одного паблишера, поэтому видится нежелательным premature-но ограничивать гибкость и возможности расширения.

Отметим очевидный факт, что в данной схеме коллизии произойдут, если в разных Fidd-ах сообщения с одним и тем же 
...
произойти никогда (т.к. номер сообщения уникален).

Т.к. футпринты - это довольно длинные строки, а также потому что мы хотим сделать как можно меньше информации 
доступной несанкционированным наблюдателям, имеет смысл скомпактировать / минимизировать размер имен Fidd Key файлов. 
Заметим, что в большинстве случаев подписчиков будет немного, а футпринты их буду разниться существенно.
...
Таким образом, использовав не весь футпринт, а только уникальный префикс в качестве имени Fidd Key файла мы не только уменьшим
размер этих имен, но и скроем большую часть информации о футпринте, еще сильнее затруднив корреляцию подписчиков между Fidd-ами.
...
использует тот же самый ключ для подписки на некие злонамеренные Fidd-ы), для подтверждения факта его подписки на другие Fidd-ы
...
Так, для десятка подписчиков они будут иметь по 2-3 символа, в то время как полный размер футпринтов будет измеряться в 
сотнях символов, и для уверенного матчинга у атакующего навряд ли хватит информации.