...
По этой причине мы также планируем разработать собственную многоуровневую модель сервисного API, в которой самая первая и базовая (а потому и самая безопасная) модель не будет иметь функций получения данных с оффсета, а только полность всего файла.  
...
- полный ли это файл или же тип diff-а (например, есть текстовый diff, binary diff, и, наверное, много что еще);
- путь к файлу / имя файла (diff будет обновлять соответствующий файл);
- тип данных файла;
- дата создания;
- дата изменения;
- (опционально) тип сжатия файла;
...
На данном этапе (версия 1.0) мы выбираем простоту и храним каждый логический файл единым куском.  
...
- версия / номер истории (для правильного применения последовательности постов-обновлений)
...
- оффсет файла (возможно также - длина файла?)
...
- Получив на вход оригинальную директорию и обновленную директорию, создает {fidd-файл и fidd.key-файл обновления}.
...
- Получив на вход оригинальную директорию и []{fidd-файл и fidd.key-файл обновлений}, обновляет оригинальную директорию.
...
- Выбор наилучшего алгоритма diff или же полного обновления файла при создании обновлений;
- Полная перепаковка поста, если количество обновлений стало слишком велико (при этом улучшится стриминг);
- и пр.
...
Действительно, какой толк знать версию формата файла без ключа, которым этот файл открывается.
...
- размер секции метаданных;
- (возможно?) размер данных / длина контента;
- полный ли это файл (new/overrride) или же | тип diff-а (например, есть текстовый diff, binary diff, etc) | или же delete флаг (файл был удален в обновлении);
- путь к файлу / имя файла (diff будет обновлять соответствующий файл);
- тип данных файла;
- дата создания;
- дата изменения;
...
- идентификатор fidd-поста (для правильного матчинга постов-обновлений)
- версия / номер истории (для правильного применения последовательности постов-обновлений)
- full_repack флаг - если пост был перепакован и должен читаться как новый
...
- Получив на вход {оригинальные fidd-файл, fidd.key-файл}, а также []{fidd-файл и fidd.key-файл обновлений}, создает перепакованный {fidd-файл и fidd.key-файл}, с контентом последней обновленной версии. При этом сохраняет идентификатор fidd-поста и увеличивает версию на 1, то есть продолжает историю. Полезно, если количество обновлений стало слишком велико (при этом улучшится стриминг);
...
- Возможность удобного расширения набора алгоритмов шифрования своими;
- Рандомизация размещения логических файлов - указать размер хедера/футера/гепа (от X байт - до Y байт);
- Выбор наилучшего варианта при создании обновлений (по размеру) - тип алгоритма diff или же полное обновление логического файла;
- и пр.
