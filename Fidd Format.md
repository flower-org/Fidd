...
### 3.1 Метаданные Fidd-файла

- версия формата метаданных fidd-файла
    - имеет смысл единая кодировка независимо от формата
    - произвольная строка

- версия формата метаданных логических файлов
  - произвольная строка
- Message Number
  - (int64, monotonically increasing)
- уникальный идентификатор fidd-поста (для правильного матчинга постов-обновлений)
  - произвольная строка
- версия / номер истории 
  - (int32, default 1, must be incremented by 1 with every update) 
  - (для правильного применения последовательности постов-обновлений)
- squash флаг - если пост был перепакован и должен читаться как новый
  - boolean
- delete флаг - если пост должен считаться удаленным
  - boolean
- (опционально) Message Number для предыдущей версии (для быстрого поиска поста в последовательности Messages)

### 3.2 Метаданные логического файла
...
- размер секции метаданных логического файла
  - int32
  - имеет смысл единая кодировка независимо от формата

- тип обновления 
  - enum
    - полный ли это файл (create/overrride) 
    - или же тип diff-а (в этом случае нужны нестандартные diff-форматы, позволяющие эффективный стриминг)
    - или же delete флаг (файл был удален в обновлении);
- тип данных файла (что-то вроде MIME-Type;
  - произвольная строка
- (опционально) дата;
  - unix time / int64?
  - в зависимости от типа обновления, дата создания или изменения.
...
### 3.4 Оффсеты, и борьба с возможностью предсказания оффсетов файлов, а также размера содержащихся в fidd-файле данных.
...
- версия формата файла fidd.key
  - имеет смысл единая кодировка независимо от формата
  - произвольная строка

Файл fidd.key должен хранить ссылку на секцию метаданных fidd-файла:

- оффсет секции метаданных fidd-файла
- длина секции метаданных fidd-файла
- (опционально) ключ шифрования (возможна структура, например, ключ + IV)
- (опционально) алгоритм шифрования
- (опционально) CRC секции метаданных fidd-файла
