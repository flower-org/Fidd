...
class EncryptionAlgorithmTest {
  ...
  void testEncryptDecryptStream(EncryptionAlgorithm encryptionAlgorithm) {
    ...
    ByteArrayInputStream inputStream =
        new ByteArrayInputStream(originalText.getBytes(StandardCharsets.UTF_8));
          ...
          assertThrows(
              ...
              () ->
                  encryptionAlgorithm.encrypt(
                      new byte[0], List.of(inputStream), new ByteArrayOutputStream(), null));
  ...
  void testStreamEncryptDecrypt(EncryptionAlgorithm encryptionAlgorithm) {
    ...
    assertArrayEquals(
        plaintext, decryptedOut.toByteArray(), "Stream decryption should restore original data");
  }
}
